...
var ppcContent = {
  ...
  showPopup: function (text, elem, x, y, looseWidth) {
    ...
    if (ppcContent.getContentType(topdoc) == 'text/plain') {
...
//			this.copyToClip();
  ...
  processEntry: function (e) {
    ...
    if ((tdata.config.highlight == 'yes') && (!('form' in tdata.prevTarget))) {
      ...
      ppcContent.highlightMatch(doc, rp, ro, e.matchLen, selEndList, tdata);
  ...
  processHtml: function (html) {
    tdata = window.cperapera;
    ppcContent.showPopup(html, tdata.prevTarget, tdata.popX, tdata.popY, false);
    return 1;
  },
